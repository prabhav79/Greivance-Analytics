    ...
    if provider == "groq":
        try:
            ...
            if r.status_code == 200:
                ...
                for preferred in ['llama-3.3-70b-versatile', 'llama3-70b-8192', 'mixtral-8x7b-32768']:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                # Look for the most advanced stable flash model
                flash_models = [m for m in models if "flash" in m and "lite" not in m and "preview" not in m and "image" not in m and "tts" not in m]
                # Sort descending to get the highest version number
                flash_models.sort(reverse=True)
                if flash_models:
                    return flash_models[0]
                if 'gemini-flash-latest' in models: return 'gemini-flash-latest'
        except:
            pass
        return "gemini-flash-latest"

def call_llm(prompt, pdf_path, provider, api_key):
    """
    Consolidated helper to call the LLM, handle rate limits, and parse JSON robustly.
    """
    if not api_key:
    ...
    try:
        ...
        result["raw_text_content"] = text_content # passing text context to caller for regex fallbacks
        return result
    except Exception as e:
        # Robust fallback for LLMs refusing to answer or giving non-JSON
        err_snippet = content[:200].replace('\n', ' ')
        return {"status": "error", "error": f"JSON Parsing Failed. LLM Output snippet: '{err_snippet}'"}
...
def analyze_atr(pdf_path, api_key="", provider="gemini"):
    ...
    result = call_llm(prompt, pdf_path, provider, api_key)
